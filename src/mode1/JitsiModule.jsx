...
export default function JitsiModule(ƒ) {
  ...
  // hosts: {
  //     domain: 'jitsi-connect.duckdns.org',
  //         muc: 'conference.jitsi-connect.duckdns.org',
  //             focus: 'focus.jitsi-connect.duckdns.org'
  // },
  // externalConnectUrl: 'https://jitsi-connect.duckdns.org/http-pre-bind',
  ...
  function onConnectionSuccess() {
    room.current = connection.current.initJitsiConference(conferenceName, confOptions);
  ...
  function onLocalTracks(tracks) {
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      tracks[i].addEventListener(
        ...
        () => console.log('local track mute changed'));
      tracks[i].addEventListener(
    ...
    room.current.on(JitsiMeetJS.events.conference.CONFERENCE_JOINED,
      () => console.log('CONNECTED with user id: ', room.current.myUserId()));
    room.current.on(JitsiMeetJS.events.conference.CONFERENCE_LEFT, cleanUp)
    room.current.on(JitsiMeetJS.events.conference.USER_JOINED, id => {
  ...
  const handleShareScreen = () => {
    ...
    JitsiMeetJS.createLocalTracks({ devices: [screenShare ? 'video' : 'desktop'] })
      .then(tracks => {
  ...
  return (
    <div style={{ marginTop: "100px" }}>
      ...
      <button onClick={handleMute}>{isMuted ? 'UnMute Audio' : "Mute Audio"}</button>
      <button onClick={handleVideoMute}>{isVideoMuted ? 'UnMute Video' : "Mute Video"}</button>
